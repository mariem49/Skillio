...
export class MatchingService {
    ...
    calculateMatchingScore(offer: JobOffer): number {
        ...
        // 1. Skills (40%)
        const matchingSkills = offer.requiredSkills.filter(skill =>
            this.studentProfile.skills.includes(skill)
        );
        const skillsPercent = offer.requiredSkills.length > 0
            ? (matchingSkills.length / offer.requiredSkills.length) * 100
            : 100;

        score += skillsPercent * 0.4;

        // 2. Education (20%)
        const educationMatch = this.studentProfile.level === offer.requiredLevel ? 100 : 50;
        // Simplified logic: Exact match = 100, else 50
        score += educationMatch * 0.2;

        // 3. Experience (20%)
        const experienceMatch = this.studentProfile.experience >= offer.experienceYears ? 100 : 60;
        score += experienceMatch * 0.2;

        // 4. Location (10%)
        const locationMatch = (offer.remote === 'full-remote' || offer.location === this.studentProfile.location) ? 100 : 40;
    ...
    getMatchingReasons(offer: JobOffer): string[] {
        ...
        if (this.studentProfile.level === offer.requiredLevel) {
            reasons.push("Education level matches");
        }

        if (this.studentProfile.experience >= offer.experienceYears) {
            reasons.push("Required experience met");
        }

        if (offer.remote === 'full-remote' || offer.location === this.studentProfile.location) {
