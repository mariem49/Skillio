...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatButtonModule, MatIconModule, MatSlideToggleModule, FormsModule],
  template: `
    <div class="max-w-6xl mx-auto p-6">
      <div class="mb-10 text-center">
        <h1 class="text-3xl font-bold text-gray-900 mb-2">Subscription & Billing</h1>
        <p class="text-gray-500">Manage your plan and billing details</p>
      </div>
      ...
      @if (loading()) {
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-violet-100 overflow-hidden max-w-3xl mx-auto">
          <div class="bg-violet-600 p-8 text-white relative overflow-hidden">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          @for (plan of plans(); track plan.id) {
            <div class="relative bg-white rounded-2xl border transition-all hover:-translate-y-1 hover:shadow-xl p-8 flex flex-col"
              ...
              @if (plan.highlight) {
                <div class="absolute top-0 right-0 bg-violet-600 text-white text-xs font-bold px-3 py-1 rounded-bl-xl rounded-tr-xl">
                  RECOMMENDED
                </div>
              }
              ...
              <div class="mb-6">
                <span class="text-4xl font-bold text-gray-900">
                    {{ billingCycle() === 'MONTHLY' ? (plan.monthlyPrice | currency) : (plan.yearlyPrice | currency) }}
                </span>
              ...
              <button mat-flat-button color="primary" (click)="subscribe(plan)" 
                      class="w-full py-3 h-auto rounded-xl font-bold"
                      [class.bg-violet-600]="plan.highlight"
                      [class.bg-gray-900]="!plan.highlight">
                Choose {{ plan.name }}
              </button>
            </div>
          }
...
export class EnterpriseSubscriptionComponent implements OnInit {
  ...
  subscribe(plan: PricingPlan) {
    ...
    if (confirm(`Subscribe to ${plan.name} (${this.billingCycle()})?`)) {
      this.pricingService.subscribe(userId, 'ENTERPRISE', plan.id, this.billingCycle()).subscribe({
        next: (sub: Subscription) => {
          this.currentSubscription.set(sub);
