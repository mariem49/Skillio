...
@Component({
   selector: 'app-trainer-subscription',
   standalone: true,
   imports: [CommonModule, MatButtonModule, MatIconModule, FormsModule],
   template: `
    <div class="max-w-6xl mx-auto p-6">
      <div class="mb-10 text-center">
        <h1 class="text-3xl font-bold text-gray-900 mb-2">Trainer Pro</h1>
        <p class="text-gray-500">Upgrade to reach more students and get advanced analytics</p>
      </div>
      ...
      @if (loading()) {
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-violet-100 overflow-hidden max-w-3xl mx-auto">
          <div class="bg-gradient-to-r from-orange-500 to-amber-500 p-8 text-white relative overflow-hidden">
...
export class TrainerSubscriptionComponent implements OnInit {
   ...
   subscribe(plan: PricingPlan) {
      ...
      if (confirm(`Subscribe to ${plan.name} (${this.billingCycle()})?`)) {
         this.pricingService.subscribe(userId, 'TRAINER', plan.id, this.billingCycle()).subscribe({
